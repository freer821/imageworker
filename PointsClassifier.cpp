...
PointsClassifier::PointsClassifier(const string & img_name) {
	initParams(imread(img_name,0));
}

PointsClassifier::PointsClassifier(const Mat & img)
{
	initParams(img);
}
...
void PointsClassifier::initParams(const Mat & img)
{
	_img = img;
	_imgH = img.size().height;
	_imgW = img.size().width;
	_cal_img_space = Mat(_imgH + 2, _imgW + 2, CV_8UC1, Scalar(0));
	for (int y = 0; y < _imgH; y++) {
		for (int x = 0; x < _imgW; x++) {
			if (_img.at<uchar>(x, y) == 255) {
